...
namespace System.IO
{
    public abstract partial class Stream : MarshalByRefObject, IDisposable, IAsyncDisposable
    {
        ...
        private static void OnCancelReadWrite(object? obj)
        {
            ...
            uint threadId = (uint)thisTask._osThreadId;
            if (threadId == 0)
                return;
            IntPtr hThread = Interop.Kernel32.OpenThread(THREAD_TERMINATE, false, threadId);
            ...
            try
            {
                // We may hit a race between _osThreedId being set and the syscall starting, so repeat cancelling until the stream operation has returned.
